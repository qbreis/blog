...
As I am still not very familiar with types, I feel more comfortable not defining NextPage type `/* 1 */` in `blog/pages/index.tsx`, so before proceeding, I want to do small change into `blog/pages/index.tsx`:

```js
//import type { NextPage } from 'next'; /* 1 */
...
//const Home: NextPage = () => {  // arrow function vs function declaration in Next.js /* 1 */
...
I want to create new file and folder `components\Header.tsx`:
...
I do import Header component into `pages/index.tsx` `/* 1 */`:
...
I do same thing with `components\Footer.tsx`:
...
I do import Footer component into `pages/index.tsx` `/* 1 */`:
...
I create new component file`components\MetaData.tsx`:
...
import MetaData from '../components/MetaData'; /* 1 */
import Header from '../components/Header';
...
So I add file `public\favicon.svg`:
...
Following same post suggestions, as long as this same file `public\favicon.svg` is made of a single colour and placed on a transparent background, I will save it exactly as `public\mask-icon.svg`.
...
I upload this same file `public\favicon.svg` and from downloaded Zip file `favicon_io.zip` I just copy `apple-touch-icon.png` to `public/apple-touch-icon.png`.

#### 4.3.1.3 Manifest

I create new file `blog/public/manifest.json`:
...
From downloaded Zip file `favicon_io.zip` I also want to copy `android-chrome-512x512.png` to `public/android-chrome-512x512.png`.
...
In this same post its author offers me a nice tip for dark mode that I plan following, so I will update my Svg file `public/favicon.svg`:
...
Finally I do update `components/MetaData.tsx`:
...
I create new file `components\Layout.tsx`:
...
I also want to update and refactor `blog/pages/index.tsx`:
